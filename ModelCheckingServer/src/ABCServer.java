...
public class ABCServer {  
	...
	//    HashMap<String, String> packageDirectoryMap = new HashMap<String, String>();
	    ...
	    intentAppMap.put("com.twitter.android", "com.twitter.android/.StartActivity");
	    ...
	    if(args.length == 1){
	    String[] cmd = { "bash", "-c", 
        		"/home/pallavi/eclipse_workspace/ModelCheckingServer/abcStartScript.sh" + " "+
        		intentAppMap.get(args[0])};
        Process pr1 = Runtime.getRuntime().exec(cmd);
        pr1.waitFor();
	    }else{
	    	System.out.println("Give only one package name as the app to be tested");
	    	System.exit(0);
	    }
//	    
        try {  
            serverSocket = new ServerSocket(10989); //port number  
            System.out.println("Listening :on port no 10989");  
        }catch (BindException e ) {  
            System.out.println("port is already used by some other program");  
            flag=false;  
        }   
        catch (IOException e) {  
            flag=false;  
        ...
        while(flag){  
            try {  
            	System.out.println("waiting for connection from emulator");
	                ...
	                if(errCode != 2){
	                String[] cmd = { "bash", "-c", 
	                		"/home/pallavi/eclipse_workspace/ModelCheckingServer/abc.sh" + " "+
	                ...
	                while ((line=buf.readLine())!=null) {
	                	...
	                	String[] cmd = { "bash", "-c", 
	                    		"/home/pallavi/eclipse_workspace/ModelCheckingServer/abcStopScript.sh"
	                			+ " " + String.valueOf(socketConnectionCount) + " "
